...
export default function LoginPage() {
  ...
  const inp = { width:'100%', background:'#F8FAFC', border:'1.5px solid #E2E8F0', borderRadius:'10px', padding:'11px 14px 11px 40px', fontSize:'14px', color:'#0F172A', outline:'none', fontFamily:'inherit', transition:'border-color .15s' };
    ...
    <div className="auth-page">
      <div style={{width:'100%',maxWidth:'420px'}}>
        ...
        <div style={{textAlign:'center',marginBottom:'28px'}}>
          <div style={{display:'inline-flex',alignItems:'center',justifyContent:'center',width:'54px',height:'54px',borderRadius:'16px',background:'#2563EB',marginBottom:'14px',boxShadow:'0 8px 24px rgba(37,99,235,.35)'}}>
          ...
          <div style={{fontSize:'14px',color:'#64748B',marginTop:'4px'}}>Your intelligent finance companion</div>
        </div>

        <div className="auth-card">
          <div style={{marginBottom:'22px'}}>
            <div style={{fontSize:'20px',fontWeight:'700',color:'#0F172A'}}>Welcome back</div>
            <div style={{fontSize:'13px',color:'#64748B',marginTop:'3px'}}>Sign in to your account</div>
          </div>
          ...
          <form onSubmit={handleSubmit}>
            <div style={{marginBottom:'14px'}}>
              ...
              <div style={{position:'relative'}}>
                ...
                <input type="email" value={form.email} onChange={e=>setForm(p=>({...p,email:e.target.value}))} style={inp} placeholder="you@example.com" required/>
              </div>
            </div>
            ...
            <div style={{marginBottom:'20px'}}>
              ...
              <div style={{position:'relative'}}>
                ...
                <input type={showPw?'text':'password'} value={form.password} onChange={e=>setForm(p=>({...p,password:e.target.value}))} style={{...inp,paddingRight:'42px'}} placeholder="••••••••" required/>
            ...
            <button type="submit" disabled={loading} style={{width:'100%',background:'#2563EB',color:'#fff',fontWeight:'700',fontSize:'15px',padding:'13px',borderRadius:'11px',border:'none',cursor:loading?'not-allowed':'pointer',display:'flex',alignItems:'center',justifyContent:'center',gap:'8px',opacity:loading?.7:1,transition:'background .15s',fontFamily:'inherit'}}>
              {loading
                ? <div className="spinner" style={{width:'18px',height:'18px',border:'2px solid rgba(255,255,255,.3)',borderTop:'2px solid #fff'}}/>
                : <>Sign in <svg width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M13 7l5 5m0 0l-5 5m5-5H6"/></svg></>
              }
            </button>
          </form>
        ...
        <div style={{textAlign:'center',fontSize:'12px',color:'#94A3B8',marginTop:'16px'}}>
          Secured with JWT · Data stored locally
        </div>
      </div>
    </div>
  );
